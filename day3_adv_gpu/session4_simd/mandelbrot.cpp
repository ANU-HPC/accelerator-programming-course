        ...
        for (j = 0, y = _mm_add_ps(_mm_set1_ps(-0.2f),_INIT_Y_4); j < DIMY/2; j += 4, y += _F_STEP_Y) {
            ...
            while (scalar_iter < 256) {
                ...
                if (_mm_test_all_zeros(_mm_castps_si128(vmask), _mm_castps_si128(vmask))){
                    ...
                    // Blended flavour of the code, this code blends values from
                    //previous iteration with the values
                    // from current iteration. Only values which did not hit the
                    //“exit” condition are being stored;
                    // values which are already “out” are maintaining their
                    //value
